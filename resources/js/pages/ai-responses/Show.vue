...
const ignoredFields = computed(() =>
    stringList(parsedAnswer.value?.ignored_fields),
);
...
function annotationExtraEntries(annotation: Annotation): Array<[string, unknown]> {
    const knownKeys = new Set([
        'tag',
        'label',
        'category',
    ...
    <div class="flex flex-col gap-6">
        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div class="rounded-xl border p-4">
                ...
                <Badge
                    class="mt-2"
                    :variant="aiResponse.data.status === 'error' ? 'destructive' : aiResponse.data.status === 'pending' ? 'secondary' : 'default'"
                >
                    {{ aiResponse.data.status }}
                </Badge>
            </div>
            ...
            <div class="rounded-xl border p-4">
                ...
                <p class="mt-1 font-medium capitalize">{{ aiResponse.data.entity }}</p>
            </div>
            ...
            <div class="rounded-xl border p-4">
                ...
                <p class="mt-1 text-sm font-medium">{{ aiResponse.data.created_at }}</p>
            </div>
        </div>
        ...
        <Card v-if="aiResponse.data.answer">
            <CardHeader>
                ...
                <div class="flex flex-col gap-1">
                    <p class="text-sm text-muted-foreground">Structured answer</p>
                    <CardTitle class="text-lg">
                        {{ textValue(parsedAnswer?.summary, 'Annotation recommendations') }}
                    </CardTitle>
                    <CardDescription v-if="parsedAnswer?.model || parsedAnswer?.entity_type">
                        {{ textValue(parsedAnswer?.model, textValue(parsedAnswer?.entity_type)) }}
                    </CardDescription>
                </div>
                ...
                <div v-if="parsedAnswer" class="flex flex-wrap gap-2">
                    ...
                    <Badge v-if="uncertainAnnotations.length" variant="secondary">
            ...
            <CardContent v-if="parsedAnswer" class="grid gap-6">
                <div class="grid gap-3 sm:grid-cols-2">
                    <div class="rounded-lg bg-muted/40 p-4">
                        ...
                        <p class="mt-1 text-2xl font-semibold">{{ annotations.length }}</p>
                    </div>
                    <div class="rounded-lg bg-muted/40 p-4">
                        <p class="text-sm text-muted-foreground">Ignored fields</p>
                        <p class="mt-1 text-2xl font-semibold">{{ ignoredFields.length }}</p>
                    </div>
                </div>
                ...
                <div v-if="annotations.length" class="grid gap-3">
                    <div>
                        ...
                        <p class="text-sm text-muted-foreground">
                            Expand an annotation to see why it was suggested and which fields support it.
                        </p>
                    </div>
                    ...
                    <div class="grid gap-3 lg:grid-cols-2">
                        <Collapsible
                            ...
                            :default-open="annotationIndex === 0"
                            class="rounded-lg border bg-background"
                        >
                            <CollapsibleTrigger as-child>
                                <button type="button" class="group flex w-full items-start justify-between gap-4 p-4 text-left">
                                <div class="min-w-0 flex flex-col gap-1">
                                    <div class="flex flex-wrap items-center gap-2">
                                        <span class="font-medium">
                                            {{ textValue(annotation.label, textValue(annotation.tag, 'Untitled annotation')) }}
                                        </span>
                                        <Badge v-if="annotation.category" variant="outline">
                                            {{ annotation.category }}
                                        </Badge>
                                    </div>
                                    <p v-if="annotation.tag" class="truncate font-mono text-xs text-muted-foreground">
                                        {{ annotation.tag }}
                                    </p>
                                </div>

                                <div class="flex shrink-0 items-center gap-2">
                                    <Badge>{{ confidenceLabel(annotation.confidence) }}</Badge>
                            ...
                            <div class="grid gap-4 border-t p-4 text-sm">
                                <div v-if="confidencePercent(annotation.confidence) !== null" class="grid gap-2">
                                    <div class="flex items-center justify-between gap-3 text-muted-foreground">
                                        <span>Confidence</span>
                                        <span class="font-medium text-foreground">{{ confidenceLabel(annotation.confidence) }}</span>
                                    </div>
                                    <div class="h-2 overflow-hidden rounded-full bg-muted">
                                        <div
                                            class="h-full rounded-full bg-primary transition-[width]"
                                            :style="{ width: `${confidencePercent(annotation.confidence)}%` }"
                                        />
                                    </div>
                                </div>

                                <dl class="grid gap-3 sm:grid-cols-2">
                                    <div v-if="annotation.tag" class="grid gap-1">
                                        <dt class="text-xs font-medium uppercase tracking-wide text-muted-foreground">Tag</dt>
                                        <dd class="font-mono text-xs">{{ annotation.tag }}</dd>
                                    </div>
                                    <div v-if="annotation.existing_tag !== undefined" class="grid gap-1">
                                        <dt class="text-xs font-medium uppercase tracking-wide text-muted-foreground">Existing tag</dt>
                                        <dd>{{ annotation.existing_tag ? 'Yes' : 'No' }}</dd>
                                    </div>
                                </dl>

                                <div v-if="annotation.reason" class="grid gap-1">
                                    <p class="text-xs font-medium uppercase tracking-wide text-muted-foreground">Why this was suggested</p>
                                    <p>{{ annotation.reason }}</p>
                                </div>

                                <div v-if="sourceFields(annotation).length" class="grid gap-2">
                                    <p class="text-xs font-medium uppercase tracking-wide text-muted-foreground">Source fields</p>
                                    <div class="flex flex-wrap gap-2">
                                        <Badge
                                            v-for="field in sourceFields(annotation)"
                                            :key="field"
                                            variant="secondary"
                                        >
                                            {{ field }}
                                ...
                                <Collapsible v-if="annotationExtraEntries(annotation).length" class="rounded-md border border-dashed p-3">
                                    <CollapsibleTrigger as-child>
                                        <button type="button" class="group flex w-full items-center justify-between text-left font-medium">
                                            More details
                                            <ChevronDown class="size-4 text-muted-foreground transition-transform group-data-[state=open]:rotate-180" />
                                        </button>
                                    </CollapsibleTrigger>
                                    ...
                                    <dl class="mt-3 grid gap-3">
                                        <div v-for="[key, value] in annotationExtraEntries(annotation)" :key="key" class="grid gap-1">
                                            <dt class="text-xs font-medium uppercase tracking-wide text-muted-foreground">{{ key.replaceAll('_', ' ') }}</dt>
                                            <dd class="whitespace-pre-wrap break-words text-xs">{{ valueJson(value) }}</dd>
                                        </div>
                                    </dl>
                                    </CollapsibleContent>
                                </Collapsible>
                ...
                <div v-if="uncertainAnnotations.length" class="grid gap-3">
                    <div>
                        ...
                        <p class="text-sm text-muted-foreground">
                            These suggestions have lower certainty and should be checked before applying.
                        </p>
                    </div>

                    <div class="grid gap-3 lg:grid-cols-2">
                        <Collapsible
                            v-for="(annotation, annotationIndex) in uncertainAnnotations"
                        ...
                        >
                            <CollapsibleTrigger as-child>
                                ...
                                <div class="flex shrink-0 items-center gap-2">
                                    <Badge variant="secondary">{{ confidenceLabel(annotation.confidence) }}</Badge>
                                    <ChevronDown class="size-4 text-muted-foreground transition-transform group-data-[state=open]:rotate-180" />
                                </div>
                                </button>
                            </CollapsibleTrigger>
                            ...
                            <CollapsibleContent>
                            <div class="grid gap-4 border-t border-amber-500/20 p-4 text-sm">
                                <div v-if="annotation.reason" class="grid gap-1">
                                    <p class="text-xs font-medium uppercase tracking-wide text-muted-foreground">Why this needs review</p>
                                    <p>{{ annotation.reason }}</p>
                                </div>
                                <div v-if="sourceFields(annotation).length" class="grid gap-2">
                                    <p class="text-xs font-medium uppercase tracking-wide text-muted-foreground">Source fields</p>
                ...
                <div v-if="ignoredFields.length" class="grid gap-2 rounded-lg border border-dashed p-4">
                    <h3 class="font-medium">Ignored fields</h3>
                    <p class="text-sm text-muted-foreground">
                        The answer skipped these fields when making recommendations.
                    </p>
                    <div class="flex flex-wrap gap-2">
                        <Badge v-for="field in ignoredFields" :key="field" variant="secondary">
                            {{ field }}
                        </Badge>
                    </div>
                </div>

                <div v-if="!annotations.length && !uncertainAnnotations.length && !ignoredFields.length" class="rounded-lg border border-dashed p-4 text-sm text-muted-foreground">
                    No annotation details were returned. Open the raw JSON below to inspect the complete answer.
                </div>
                ...
                <div v-if="answerExtraEntries.length" class="grid gap-2">
                    ...
                    <dl class="grid gap-3 rounded-lg border p-4 sm:grid-cols-2">
                        <div v-for="[key, value] in answerExtraEntries" :key="key" class="grid gap-1">
                            <dt class="text-xs font-medium uppercase tracking-wide text-muted-foreground">{{ key.replaceAll('_', ' ') }}</dt>
                            <dd class="whitespace-pre-wrap break-words text-sm">{{ valueJson(value) }}</dd>
                        </div>
                    </dl>
                </div>
                ...
                <Collapsible class="rounded-lg border border-dashed p-4">
                    ...
                    <CollapsibleContent>
                        <pre class="mt-3 max-h-96 overflow-auto rounded-lg bg-muted p-4 text-xs whitespace-pre-wrap"><code>{{ formattedAnswer }}</code></pre>
                    </CollapsibleContent>
                </Collapsible>
            </CardContent>

            <CardContent v-else class="grid gap-4 p-4 text-sm text-muted-foreground">
                This answer is not valid JSON, so it is shown as plain text below.
                <pre class="mt-3 overflow-x-auto rounded-lg bg-muted p-4 text-xs whitespace-pre-wrap"><code>{{ formattedAnswer }}</code></pre>
            </CardContent>
        </Card>
